...
def create_camera_once_job(
    ...
    *,
    ...
    strategy = get_strategy_or_404(db, strategy_id)
    strategy_snapshot = build_strategy_snapshot(strategy)
    resolved_provider = model_provider or strategy.model_provider
    resolved_model = model_name or strategy.model_name
    ...
    _process_camera_once_job_inline(db, job, camera, strategy_snapshot)
    db.refresh(job)
    return serialize_job(job)
...
def _process_camera_once_job_inline(db: Session, job: Job, camera, strategy_snapshot: dict) -> None:
    storage = FileStorageService(root=camera.storage_path)
    adapter = get_provider_adapter(job.model_provider)
    ...
    started_at = time.perf_counter()
    try:
        captured_frame = capture_camera_frame(camera)
    except CameraCaptureError as exc:
        _create_failed_camera_task_record(
            db,
            job=job,
            camera_name=camera.name,
            camera_id=camera.id,
    ...
    storage_path = storage.save_bytes(captured_frame.content, captured_frame.original_name, folder="captures")
    file_asset = FileAsset(
        id=generate_id(),
...
def _create_failed_camera_task_record(
    db: Session,
    *,
    job: Job,
    camera_name: str,
    ...
    task_record = TaskRecord(
        ...
        strategy_snapshot=strategy_snapshot,
        input_file_asset_id=None,
        input_filename=f"{camera_name}.jpg",
        input_image_path="",
        preview_image_path=None,
        source_type="camera",
        camera_id=camera_id,
