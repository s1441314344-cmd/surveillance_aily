    ...
    def _post_with_retry(
        ...
        *,
        ...
        with httpx.Client(timeout=timeout_seconds) as client:
            for attempt in range(1, GOOGLE_RETRY_POLICY.max_attempts + 1):
                try:
                    response = client.post(endpoint, headers=headers, json=payload)
                    response.raise_for_status()
                    return response.json()
                except httpx.HTTPStatusError as exc:
                    if not GOOGLE_RETRY_POLICY.should_retry(exc, attempt=attempt):
                        raise
                    last_exc = exc
                    time.sleep(GOOGLE_RETRY_POLICY.delay_seconds(exc, attempt=attempt))
                except httpx.HTTPError as exc:
                    if not GOOGLE_RETRY_POLICY.should_retry(exc, attempt=attempt):
                        raise
                    last_exc = exc
        ...
        if last_exc is not None:
            raise last_exc
        raise RuntimeError("Google request failed without an exception")
